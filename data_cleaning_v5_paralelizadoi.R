...
carpeta_salida_removed_ids <- "./Datos/archive/removed_ids_datacleaning"
...
procesar_archivo <- function(archivo, carpeta_salida, carpeta_salida_removed_ids, weather_hourly, informations_households) {
  # Leer el archivo
  ...
  ## Eliminamos aquellos hogares con + del 10% de NA's, y los guardamos en un dataset removed_ids = ids_removed_cleaning_block_xxx
  if(any(pct_na_hogar$eliminar_por_exceso_nas)){
    removed_ids <- data.frame(id_removed = rep(0, nrow(pct_na_hogar[pct_na_hogar$eliminar_por_exceso_nas == TRUE, ])), 
                              block = rep(0, nrow(pct_na_hogar[pct_na_hogar$eliminar_por_exceso_nas == TRUE, ])))
    removed_ids$id_removed <- pct_na_hogar %>% filter(eliminar_por_exceso_nas == TRUE) %>% pull(lc_lid)
    removed_ids$block <- sub("\\.csv$", "", basename(archivo))
    removed_ids$motive <- "NA excess"
  }
  ...
  # Guardar ID's que se eliminarán por tener > de 10% de atípicos
  if(any(pct_atipicos_ID$eliminar_exceso_atipicos)){
    removed_ids_outliers <- data.frame(id_removed = rep(0, nrow(pct_atipicos_ID[pct_atipicos_ID$eliminar_exceso_atipicos == TRUE, ])), 
                                       block = rep(0, nrow(pct_atipicos_ID[pct_atipicos_ID$eliminar_exceso_atipicos == TRUE, ])))
    removed_ids_outliers$id_removed <- pct_atipicos_ID %>% filter(eliminar_exceso_atipicos == TRUE) %>% pull(lc_lid)
    removed_ids_outliers$block <- sub("\\.csv$", "", basename(archivo))
    removed_ids_outliers$motive <- "Outlier Excess"
    nombre_archivo_salida <- paste("removed_ids", basename(archivo), sep = "_")
    # Unir CSV's de ID's a eliminar
    if(!exists("removed_ids")){
      removed_ids <- removed_ids_outliers
      write_csv(removed_ids, file.path(carpeta_salida_removed_ids, nombre_archivo_salida))
    } else {
      removed_ids <- rbind(removed_ids, removed_ids_outliers)
      write_csv(removed_ids, file.path(carpeta_salida_removed_ids, nombre_archivo_salida))
    }
  }
  
  
  # Filtrar fuera atípicos y guardar los documentos
...
clusterExport(cl, c("archivos", "carpeta_salida", "carpeta_salida_removed_ids", "weather_hourly", "informations_households", 
...
parLapply(cl, archivos, procesar_archivo, carpeta_salida, carpeta_salida_removed_ids, weather_hourly, informations_households)

# Detener el clúster
stopCluster(cl)


# Cuando estén todos los csvs de los ID's eliminados, juntar en un solo csv
# Añadir información de tipo de outlier en informations_households columna outlier_excess_data_cleaning y NA_excess_data_cleaning (booleanas)
# Subir luego informations_household_MOD (no en el normal, que se usa para la matriz gamma del clustering) modificado al server

archivos <- list.files(carpeta_salida_removed_ids, full.names = TRUE)
#archivos <- archivos[1]
...
ids_removed_datos_hourly <- do.call(rbind, data_list)
table(ids_removed_datos_hourly$motive) %>% prop.table()
rm(data_list)